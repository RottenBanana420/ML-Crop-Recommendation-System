"""
Comprehensive Test Suite for XGBoost Model
...
class TestXGBoostModel:
    """Test suite for XGBoost crop recommendation model."""
    ...
    def model_artifacts(self, project_paths):
        ...
        assert model_path.exists(), "XGBoost model file does not exist. Run training first."
        ...
        # Load full metrics
        metrics_path = models_dir / 'xgboost_full_metrics.json'
    ...
    # ==================== Test 4: Confusion Matrix Validation ====================
    
    def test_confusion_matrix_dimensions(self, model_artifacts):
        """
        FAIL if confusion matrix dimensions are incorrect.
        
        Should be (22, 22) for 22 crop classes.
        """
        cm = np.array(model_artifacts['metrics']['confusion_matrix'])
        n_classes = len(model_artifacts['label_encoder'].classes_)
        
        assert cm.shape == (n_classes, n_classes), (
            f"Confusion matrix has shape {cm.shape}, expected ({n_classes}, {n_classes})."
        )
    
    def test_confusion_matrix_row_sums(self, model_artifacts, test_data):
        """
        FAIL if confusion matrix row sums don't match actual class counts.
        
        Each row should sum to the number of samples in that class.
        """
        cm = np.array(model_artifacts['metrics']['confusion_matrix'])
        _, y_test = test_data
        ...
        for i in range(cm.shape[0]):
            row_sum = cm[i].sum()
            if i in actual_counts:
                assert row_sum == actual_counts[i], (
                    f"Confusion matrix row {i} sums to {row_sum}, "
                    f"but actual class count is {actual_counts[i]}."
                )
    
    def test_confusion_matrix_no_negatives(self, model_artifacts):
        """
        FAIL if confusion matrix contains negative values.
        ...
        assert np.all(cm >= 0), (
            f"Confusion matrix contains negative values. "
            f"This should never happen."
        )
    
    def test_confusion_matrix_diagonal_dominance(self, model_artifacts):
        """
        FAIL if diagonal sum < 95% of total predictions.
        
        Most predictions should be correct (on diagonal).
        """
        cm = np.array(model_artifacts['metrics']['confusion_matrix'])
        ...
        assert diagonal_ratio >= 0.95, (
            f"Diagonal sum is {diagonal_ratio:.4f} of total, expected >= 0.95. "
            f"Too many misclassifications."
        )
    
    # ==================== Test 5: Prediction Sanity ====================
    
    def test_predictions_are_valid_classes(self, model_artifacts, test_data):
        """
        FAIL if model produces invalid class predictions.
        
        All predictions must be within valid class range [0, 21].
        """
        X_test, _ = test_data
    ...
    def test_model_file_exists(self, project_paths):
        ...
        """
        model_path = project_paths['models_dir'] / 'xgboost_model.pkl'
    ...
    def test_model_handles_all_zeros(self, model_artifacts, test_data):
        """
        FAIL if model crashes on all-zeros input.
        ...
        # Create all-zeros input
        X_zeros = pd.DataFrame(np.zeros((1, X_test.shape[1])), columns=X_test.columns)
        
        try:
            prediction = model.predict(X_zeros)
            assert len(prediction) == 1
            # Prediction should be valid class
            n_classes = len(model_artifacts['label_encoder'].classes_)
            assert 0 <= prediction[0] < n_classes
        except Exception as e:
            pytest.fail(f"Model failed on all-zeros input: {str(e)}")
    
    def test_model_handles_extreme_outliers(self, model_artifacts, test_data):
        """
        FAIL if model crashes on extreme outliers (3σ beyond training range).
        ...
        # Create extreme outliers (3 standard deviations beyond max)
        X_mean = X_test.mean()
        X_std = X_test.std()
        X_outlier = pd.DataFrame([(X_mean + 3 * X_std).values], columns=X_test.columns)
        
        try:
            prediction = model.predict(X_outlier)
            assert len(prediction) == 1
            # Prediction should be valid class
            n_classes = len(model_artifacts['label_encoder'].classes_)
            assert 0 <= prediction[0] < n_classes
        except Exception as e:
            pytest.fail(f"Model failed on extreme outliers: {str(e)}")
    ...
    def test_feature_importance_sum(self, model_artifacts):
        ...
        XGBoost importances should sum to 1.0.
        """
    ...
    def test_no_negative_importances(self, model_artifacts):
        ...
        assert all(imp >= 0 for imp in importances), (
            ...
            f"This should never happen with XGBoost."
        )
